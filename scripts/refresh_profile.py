...
def verify_facts(cfg, rep, workdir: Path):
    ...
    # ---- projects: exact, cross-checked two ways ----
    ...
    if spec.get("derive", {}).get("method") == "exact":
        ...
        if str(n_products) != declared:
            ...
            print(f"    derived  projects = {n_products}  (from products[])")
        rep.checks += 1
    ...
    if d.get("method") in ("floor", "floor_display"):
        ...
        if missing:
            rep.warn("verify", "tests",
                     f"could not fetch {', '.join(missing)}; floor is incomplete")
        gap = declared - floor
